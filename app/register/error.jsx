// app/register/error.jsx
"use client";

import { useEffect } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { UserPlus, Home, RefreshCw, LogIn, AlertCircle } from "lucide-react";
import * as Sentry from "@sentry/nextjs";

/**
 * Composant Error Boundary pour la route /register
 * Capture toutes les erreurs non gérées de la page d'inscription
 *
 * Contexte spécifique:
 * - Erreur lors du chargement/affichage de la page d'inscription
 * - Point d'entrée pour nouveaux utilisateurs
 * - Erreur empêche l'accès au formulaire d'inscription
 *
...
export default function RegisterError({ error, reset }) {
  ...
  useEffect(() => {
    ...
    Sentry.captureException(error, {
      tags: {
        error_boundary: "register",
        component: "register-error",
        page: "/register",
        critical: true, // Critique car empêche l'inscription
        business_impact: "high", // Impact élevé sur l'acquisition utilisateur
      },
      contexts: {
        registration: {
          description: "Erreur lors de l'affichage de la page d'inscription",
          userAction: "Tentative d'accès à la page d'inscription",
          impact:
            "L'utilisateur ne peut pas accéder au formulaire d'inscription",
        },
      },
      extra: {
        digest: error.digest,
    ...
    console.error("Register Error Boundary caught error:", {
      name: error.name,
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full">
        {/* Icône d'erreur avec inscription */}
        <div className="flex justify-center mb-6">
          <div className="relative">
            <div className="bg-red-100 rounded-full p-6">
              <UserPlus className="text-red-600" size={48} strokeWidth={1.5} />
            </div>
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-center text-gray-900 mb-3">
          Erreur lors du chargement de la page d&apos;inscription
        </h1>

        {/* Message */}
        <p className="text-center text-gray-600 mb-8 leading-relaxed">
          Nous sommes désolés, une erreur inattendue s&apos;est produite lors du
          chargement de la page d&apos;inscription. Notre équipe a été
        ...
        <div className="space-y-3">
          ...
          {/* Bouton Se connecter - Alternative si l'utilisateur a déjà un compte */}
          <Link
            href="/login"
        ...
        <div className="mt-6 p-4 bg-amber-50 rounded-lg border border-amber-100">
          ...
          <p className="text-sm text-amber-800 leading-relaxed">
            Si vous avez déjà créé un compte auparavant, vous pouvez{" "}
            <Link
              href="/login"
